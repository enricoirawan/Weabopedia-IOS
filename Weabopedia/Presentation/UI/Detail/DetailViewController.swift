...
class DetailViewController: UIViewController {
    ...
    var insertAnimeToFavoriteUseCase: InsertAnimeToFavoriteUseCase?
    var deleteAnimeFromFavoriteUseCase: DeleteAnimeFromFavoriteUseCase?
    ...
    private func checkFavorite() {
        ...
        checkIsFavoriteUseCase?.execute(with: animeId)
    ...
    private func insertToFavorite() {
        ...
        insertAnimeToFavoriteUseCase?.execute(with: anime)
            .receive(on: RunLoop.main)
            .sink(receiveCompletion: { completion in
                switch completion {
                case .failure:
    ...
    private func deleteFromFavorite() {
        guard let anime = detail else { return }
        deleteAnimeFromFavoriteUseCase?.execute(with: anime.id)
            .receive(on: RunLoop.main)
            .sink(receiveCompletion: { completion in
                switch completion {
                case .failure:
